            ...
            @forelse($stockTransactions as $stockTransaction)
                <tr wire:key="stockTransaction-{{ $stockTransaction->id }}" class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm font-medium text-gray-900">{{ $stockTransaction->transaction_code }}</div>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if($stockTransaction->type->value === 'in')
                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-100 text-green-800 rounded-full text-xs font-semibold">
                        ...
                        @else
                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-100 text-red-800 rounded-full text-xs font-semibold">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 20V4m0 0l4 4m-4-4l-4 4"/>
                                </svg>
                                Stok Keluar
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if($stockTransaction->type->value === 'in')
                            <div class="flex items-center gap-2">
                                <span class="text-2xl font-bold text-green-600">+{{ number_format($stockTransaction->quantity) }}</span>
                                <span class="text-xs text-gray-500">unit</span>
                            </div>
                        @else
                            <div class="flex items-center gap-2">
                                <span class="text-2xl font-bold text-red-600">-{{ number_format($stockTransaction->quantity) }}</span>
                                <span class="text-xs text-gray-500">unit</span>
                            </div>
                        @endif
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                        <button
                            wire:click="openEditModal({{ $stockTransaction->id }})"
                            class="text-indigo-600 hover:text-indigo-900"
                        >
                            Edit
                        </button>
                        <button
                            wire:click="delete({{ $stockTransaction->id }})"
            ...
            <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                <div class="p-6">
                    ...
                    <form wire:submit="save" class="space-y-4">
                        ...
                        <!-- Type -->
                        <div>
                            <label class="block mb-2.5 text-sm font-medium text-heading">Tipe Transaksi</label>
                            <div class="grid grid-cols-2 gap-4">
                                <label class="relative flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-50 {{ $type === 'in' ? 'border-green-500 bg-green-50' : 'border-gray-300' }}">
                                    <input type="radio" wire:model.live="type" value="in" class="sr-only">
                                    <div class="flex items-center gap-3 w-full">
                                        <div class="flex-shrink-0">
                                            <svg class="w-8 h-8 {{ $type === 'in' ? 'text-green-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m0 0l-4-4m4 4l4-4"/>
                                            </svg>
                                        </div>
                                        <div class="flex-1">
                                            <div class="font-medium {{ $type === 'in' ? 'text-green-900' : 'text-gray-900' }}">Stok Masuk</div>
                                            <div class="text-xs {{ $type === 'in' ? 'text-green-700' : 'text-gray-500' }}">Penambahan stok</div>
                                        </div>
                                    </div>
                                </label>
                                
                                <label class="relative flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-50 {{ $type === 'out' ? 'border-red-500 bg-red-50' : 'border-gray-300' }}">
                                    <input type="radio" wire:model.live="type" value="out" class="sr-only">
                                    <div class="flex items-center gap-3 w-full">
                                        <div class="flex-shrink-0">
                                            <svg class="w-8 h-8 {{ $type === 'out' ? 'text-red-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 20V4m0 0l4 4m-4-4l-4 4"/>
                                            </svg>
                                        </div>
                                        <div class="flex-1">
                                            <div class="font-medium {{ $type === 'out' ? 'text-red-900' : 'text-gray-900' }}">Stok Keluar</div>
                                            <div class="text-xs {{ $type === 'out' ? 'text-red-700' : 'text-gray-500' }}">Pengurangan stok</div>
                                        </div>
                                    </div>
                                </label>
                            </div>
